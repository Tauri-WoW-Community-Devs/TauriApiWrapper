...
namespace TauriApiWrapper
{
    public static class BlackMarketClient
    {
        ...
        public static async Task<ApiResponse<BlackMarketResponse>> GetBlackMarketAuctionsAsync(TauriClient client, Realm realm, CancellationToken cancellationToken = default)
        {
            if (realm.GetRealmExpansion() < Expansion.MistsOfPandaria)
                return new ApiResponse<BlackMarketResponse>() { ErrorMessage = TauriClient.InvalidExpansion };

            ApiParams param = new ApiParams(Endpoints.AuctionsData, client.ApiSecret, new AuctionHouseBaseRequest(realm));
            return await client.CommunicateAsync<BlackMarketResponse>(param, cancellationToken);
        }

        public static async Task<ApiResponse<AuctionHouseInfoResponse>> GetBlackMarketInfoAsync(TauriClient client, Realm realm, CancellationToken cancellationToken = default)
        {
            if (realm.GetRealmExpansion() < Expansion.MistsOfPandaria)
                return new ApiResponse<AuctionHouseInfoResponse>() { ErrorMessage = TauriClient.InvalidExpansion };
